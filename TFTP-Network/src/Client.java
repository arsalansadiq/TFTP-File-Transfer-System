	...
	private ByteArrayOutputStream getFile() throws IOException {
		...
		do {
			...
			if (requestCode[0] == 0 && requestCode[1] == 5) {
				errorOccurred(receivePacket);
				sendReceiveSocket.receive(receivePacket);
				System.out.println("Client received packet after sending error to thread: " + receivePacket.getData()[0] + receivePacket.getData()[1]
			...
			if (requestCode[1] == 3) { // 3 is opcode for data in packet
				...
				if (blockNum == actualBlockNum) {
					...
					sendReceiveSocket.receive(receivePacket);
					System.out.println("Client received packet during correct block: " + receivePacket.getData()[0] + receivePacket.getData()[1]
							+ " with block number " + receivePacket.getData()[2] + receivePacket.getData()[3]);
					...
					blockNum--;
					sendReceiveSocket.receive(receivePacket);
					System.out.println("Client received packet during wrong block: " + receivePacket.getData()[0] + receivePacket.getData()[1]
							+ " with block number " + receivePacket.getData()[2] + receivePacket.getData()[3]);
					//acknowledgeToHost(byteArrToInt(blockNumber));
				}
			}
		...
		System.out.println("Client exiting transfer: " + receivePacket.getData()[0] + receivePacket.getData()[1]
				+ " with block number " + receivePacket.getData()[2] + receivePacket.getData()[3] + " with size: " + receivePacket.getLength());
